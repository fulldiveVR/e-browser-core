...
const start = (passthroughArgs, buildConfig = config.defaultBuildConfig, options) => {
  ...
  if (options.user_data_dir_name) {
    if (process.platform === 'darwin') {
      user_data_dir = path.join(process.env.HOME, 'Library', 'Application\\ Support', 'TauTechnologies', options.user_data_dir_name)
    } else if (process.platform === 'win32') {
      user_data_dir = path.join(process.env.LocalAppData, 'TauTechnologies', options.user_data_dir_name)
    } else {
      user_data_dir = path.join(process.env.HOME, '.config', 'TauTechnologies', options.user_data_dir_name)
    }
  ...
  if (!outputPath) {
    outputPath = path.join(config.outputDir, 'taubrowser')
