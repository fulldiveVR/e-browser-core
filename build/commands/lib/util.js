...
const util = {
  ...
  updateMidlFiles: () => {
    Log.progressScope('update midl files', () => {
      const files = fs.readdirSync(path.join(config.braveCoreDir, 'win_build_output', 'midl'))
      for (const file of files) {
        const srcFile = path.join(config.braveCoreDir, 'win_build_output', 'midl', file)
        const dstFile = path.join(config.srcDir, 'third_party', 'win_build_output', 'midl', file)
        try {
          const stat = fs.lstatSync(srcFile);
          // only copy the directories here
          // they each have a structure with x86/x64/arm64 versions of the COM files
          if (stat.isDirectory()) {
            fs.copySync(srcFile, dstFile)
          }
        } catch (e) {
          console.log('error copying file \"' + srcFile + "\"  to \"" + dstFile + "\"")
        }
      }
    })
  },
