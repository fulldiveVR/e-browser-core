...
void BraveNewsController::ConditionallyStartOrStopTimer() {
  ...
  MaybeInitPrefs();
  // Refresh data on an interval only if AI Wize News is enabled
  if (GetIsEnabled(prefs_)) {
...
void BraveNewsController::MaybeInitPrefs() {
  if (GetIsEnabled(prefs_)) {
    ...
    // locale in earlier versions of AI Wize News. If so, we should remove it.
...
void BraveNewsController::OnNetworkChanged(
  ...
  // handles the case where AI Wize News is started with no network.
