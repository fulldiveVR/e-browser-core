...
export function useAssetManagement() {
  ...
  const onAddUserAsset = React.useCallback(
    async (token: BraveWallet.BlockchainToken) => {
      ...
      if (
        userTokensRegistry &&
        isTokenRemovedInLocalStorage(assetId, userTokensRegistry.hiddenTokenIds)
      ) {
  ...
  const onUpdateVisibleAssets = React.useCallback(
      ...
      await eachLimit(
        ...
        async (token) => {
          ...
          if (
            ...
            !isTokenRemovedInLocalStorage(
              tokenId,
              userTokensRegistry.hiddenTokenIds
            )
          ) {
    ...
    [userVisibleTokensInfo, hideOrDeleteToken]
  )
