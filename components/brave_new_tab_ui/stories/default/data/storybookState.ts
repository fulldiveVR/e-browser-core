...
export const useNewTabData = (state: NewTab.State = defaultState) => {
  const result: NewTab.State = {
    ...
    braveRewardsSupported: boolean('Brave Rewards supported?', false),
    showRewards: boolean('Show rewards?', false),
    showBraveTalk: boolean('Show Brave Talk?', false),
    braveTalkSupported: boolean('Brave Talk supported?', false),
    hideAllWidgets: boolean('Hide all widgets?', true),
