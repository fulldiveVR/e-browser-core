...
constexpr char kPrimaryLanguageHistogramName[] = "Brave.Core.PrimaryLang";
...
void LanguageMetrics::RecordLanguageMetric() {
  ...
  if (language_it == kOrderedLangCodes.end()) {
    // language is not in list, suspend metric
    UMA_HISTOGRAM_EXACT_LINEAR(kPrimaryLanguageHistogramName, INT_MAX - 1,
                               kOrderedLangCodes.size());
    return;
  }

  int index = std::distance(kOrderedLangCodes.begin(), language_it);
  UMA_HISTOGRAM_EXACT_LINEAR(kPrimaryLanguageHistogramName, index,
                             kOrderedLangCodes.size());
}
