...
EngineConsumerOpenAIRemote::EngineConsumerOpenAIRemote(
  ...
  base::flat_set<std::string_view> stop_sequences(kStopSequences.begin(),
                                                  kStopSequences.end());
  apiOpenAI = std::make_unique<RemoteCompletionOpenAIClient>(
...
EngineConsumerOpenAIRemote::~EngineConsumerOpenAIRemote() = default;

void EngineConsumerOpenAIRemote::ClearAllQueries() {
  apiOpenAI->ClearAllQueries();
}

void EngineConsumerOpenAIRemote::GenerateQuestionSuggestions(
    const bool& is_video,
  ...
  DCHECK(apiOpenAI);
  apiOpenAI->QueryPrompt(
      prompt, stop_sequences,
      base::BindOnce(
          &EngineConsumerOpenAIRemote::OnGenerateQuestionSuggestionsResponse,
          weak_ptr_factory_.GetWeakPtr(), std::move(callback)));
}

void EngineConsumerOpenAIRemote::OnGenerateQuestionSuggestionsResponse(
    SuggestedQuestionsCallback callback,
...
void EngineConsumerOpenAIRemote::GenerateAssistantResponse(
    const bool& is_video,
    const std::string& page_content,
  ...
  DCHECK(apiOpenAI);
  apiOpenAI->QueryPrompt(prompt, {"</response>"}, std::move(completed_callback),
                    std::move(data_received_callback));
}

void EngineConsumerOpenAIRemote::SanitizeInput(std::string& input) {
