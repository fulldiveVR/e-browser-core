...
void BraveRewardsSource::StartDataRequest(
  ...
  if (it != resource_fetchers_.end()) {
    LOG(WARNING) << "Already fetching specified AI Wize Rewards resource, url: "
                 << actual_url;
    return;
  }
  ...
  if (image_service) {
    net::NetworkTrafficAnnotationTag traffic_annotation =
        ...
        semantics {
          ...
          description:
            "Fetches resources related to AI Wize Rewards."
          trigger:
...
void BraveRewardsSource::OnBitmapFetched(
  ...
  if (bitmap.isNull()) {
    LOG(ERROR) << "Failed to retrieve AI Wize Rewards resource, url: " << url;
    std::move(got_data_callback).Run(nullptr);
    return;
  }
