...
// URL rather than showing tau://wallet for everything. This is needed
// because of a side effect from rewriting tau:// to chrome:// in
// HandleURLRewrite handler which makes tau://wallet the virtual URL here
// unless we return true to trigger an update of virtual URL here to the routed
// URL. For example, we will display tau://wallet/send instead of
// tau://wallet with this. This is Android only because currently both
...
// Also update the real URL from tau:// to chrome://.
#if BUILDFLAG(IS_ANDROID)
  if ((url->SchemeIs(content::kBraveUIScheme) ||
       url->SchemeIs(content::kChromeUIScheme)) &&
      url->host() == kWalletPageHost) {
...
bool BraveContentBrowserClient::HandleURLOverrideRewrite(
  ...
  // tau://sync => tau://settings/braveSync
...
#if !BUILDFLAG(IS_ANDROID)
  // tau://adblock => tau://settings/shields/filters
