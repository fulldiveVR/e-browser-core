  ...
  sectionName: string)
{
  ...
  if (!sectionEl) {
    console.error(`[AIWIZE Settings Overrides] Could not find section '${sectionName}'`)
  }
  return sectionEl
}
...
RegisterPolymerTemplateModifications({
  'settings-basic-page': (templateContent) => {
    ...
    if (!actualTemplate) {
      console.error('[AIWIZE Settings Overrides] Could not find basic-page template')
      return
    }
    const basicPageEl = actualTemplate.content.querySelector('#basicPage')
    if (!basicPageEl) {
      console.error('[AIWIZE Settings Overrides] Could not find basicPage element to insert Getting Started section')
    } else {
      const privacyGuidePromoSection = actualTemplate.content.querySelector('#privacyGuidePromoSection')
      if (!privacyGuidePromoSection) {
        console.error('[AIWIZE Settings Overrides] Could not find privacyGuidePromoSection element to hide')
      } else {
      ...
      if (!advancedTemplate) {
        console.error('[AIWIZE Settings Overrides] Could not find advanced section')
      }
      ...
      if (!advancedSubSectionsTemplate) {
        console.error('[AIWIZE Settings Overrides] Could not find advanced sub-sections container')
      }
