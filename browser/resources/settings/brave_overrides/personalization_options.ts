...
RegisterPolymerTemplateModifications({
  'settings-personalization-options': (templateContent) => {
    ...
    if (!metricsReportingControl) {
      console.error(`[AIWIZE Settings Overrides] Couldn't find metricsReportingControl`)
    } else {
    ...
    if (!searchSuggestToggleTemplate) {
      console.error('[AIWIZE Settings Overrides] Could not find searchSuggestToggle template')
    } else {
      const searchSuggestToggle = searchSuggestToggleTemplate.content.getElementById('searchSuggestToggle')
      if (!searchSuggestToggle) {
        console.error('[AIWIZE Settings Overrides] Could not find searchSuggestToggle id')
      } else {
