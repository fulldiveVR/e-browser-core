...
RegisterPolymerTemplateModifications({
  'settings-personalization-options': (templateContent) => {
    ...
    if (!metricsReportingControl) {
      console.error(`[TauBrowser Settings Overrides] Couldn't find metricsReportingControl`)
    } else {
    ...
    if (!searchSuggestToggleTemplate) {
      console.error('[TauBrowser Settings Overrides] Could not find searchSuggestToggle template')
    } else {
      const searchSuggestToggle = searchSuggestToggleTemplate.content.getElementById('searchSuggestToggle')
      if (!searchSuggestToggle) {
        console.error('[TauBrowser Settings Overrides] Could not find searchSuggestToggle id')
      } else {
