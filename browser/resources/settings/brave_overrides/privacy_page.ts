...
RegisterPolymerTemplateModifications({
  'settings-privacy-page': (templateContent) => {
    ...
    if (!pages) {
      console.error(
        `[AIWIZE Settings Overrides] Couldn't find privacy_page #pages`)
    } else {
      if (!loadTimeData.getBoolean('isIdleDetectionFeatureEnabled')) {
        ...
        if (!idleDetection) {
          console.error(
            `[AIWIZE Settings Overrides] Couldn't find idle detection template`)
        } else {
      ...
      if (!permissionsLinkRow) {
        console.error(
          '[AIWIZE Settings Overrides] Couldn\'t find permissionsLinkRow')
      } else {
      ...
      if (!thirdPartyCookiesLinkRow) {
        console.error(
          '[AIWIZE Settings Overrides] Could not find ' +
    ...
    if (!loadTimeData.getBoolean('isPrivacySandboxRestricted')) {
      ...
      if (!privacySandboxSettings3Template) {
        console.error(
          '[AIWIZE Settings Overrides] Could not find template with ' +
        ...
        if (!privacySandboxLinkRow) {
          console.error(
            '[AIWIZE Settings Overrides] Could not find privacySandboxLinkRow' +
            ' id on privacy page.')
        } else {
        ...
        if (!privacySandboxLink) {
          console.error(
            '[AIWIZE Settings Overrides] Could not find privacySandboxLink id' +
            ' on privacy page.')
        } else {
    ...
    if (!showPrivacyGuideEntryPointTemplate) {
      ...
      if (!privacyGuideLinkRow) {
        console.error(
          '[AIWIZE Settings Overrides] Could not find privacyGuideLinkRow id' +
          ' on privacy page.')
      } else {
