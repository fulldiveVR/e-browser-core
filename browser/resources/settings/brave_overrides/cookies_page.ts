...
RegisterPolymerTemplateModifications({
  'settings-cookies-page': (templateContent) => {
    ...
    if (!isNot3pcdRedesignEnabledTemplate) {
      ...
      if (!generalControls) {
        console.error(
          '[AIWIZE Settings Overrides] Could not find generalControls id ' +
          'on cookies page.')
      } else {
    ...
    if (!additionalProtections) {
      console.error(
        '[AIWIZE Settings Overrides] Could not find additionalProtections ' +
        'id on cookies page.')
    } else {
    ...
    if (!siteDataTrigger) {
      console.error(
        '[AIWIZE Settings Overrides] Could not find site-data-trigger id ' +
        'on cookies page')
    } else {
    ...
    if (!doNotTrackToggle) {
      console.error(
        '[AIWIZE Settings Overrides] Could not find toggle id on cookies page')
    } else {
...
const BraveSettingsCookiePageBehavior = {
  ready: function (this: SettingsCookiesPageElement) {
    ...
    if (!siteList) {
      throw new Error(
        '[AIWIZE Settings Overrides] Could not find allow3pcExceptionsList'
      )
    }
