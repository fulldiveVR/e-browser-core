...
RegisterPolymerTemplateModifications({
  'extensions-detail-view': (templateContent: DocumentFragment) => {
    ...
    if (!optionsTemplate) {
      console.error('[TauBrowser Extensions Overrides] Could not find optionsTemplate')
      return
    }
    ...
    if (!incognitoTemplate) {
      console.error('[TauBrowser Extensions Overrides] Could not find incognitoTemplate')
      return
    }
    let incognitoWarningDiv = incognitoTemplate.content.querySelector<HTMLDivElement>('.section-content')
    if (!incognitoWarningDiv) {
      console.error('[TauBrowser Extensions Overrides] Could not find incognitoWarningDiv')
      return
    }
