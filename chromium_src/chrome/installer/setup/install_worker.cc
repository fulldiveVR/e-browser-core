...
void AddBraveVPNHelperServiceWorkItems() {
...
bool OneTimeServiceCleanup(const base::FilePath& target_path,
  ...
  if (key.ReadValueDW(brave_vpn::kBraveVpnOneTimeServiceCleanupValue,
    ...
    if (cleanup_ran == 1) {
      VLOG(1) << "OneTimeServiceCleanup has already ran; skipping";
      return false;
    }
  }
  ...
  if (!is_test) {
    AddBraveVPNWireguardServiceWorkItems(
        brave_vpn::GetBraveVPNWireguardServiceInstallationPath(target_path,
                                                               new_version),
        install_list);
    AddBraveVPNHelperServiceWorkItems();
  }
...
#define AddUpdateDowngradeVersionItem                            \
  OneTimeServiceCleanup(target_path, new_version, install_list); \
